...
/**
 * @author Pippen
 * @since 2021/03/29 09:42
 */
@Component
@Slf4j
@AllArgsConstructor
public class MultiLineStringHandle  implements FieldsHandles {
    @Override
    public String getType() {
        return FieldsTypeEnum.multiLineString.getName();
    }

    @Override
    public TableBuilder columnBud(TableBuilder table, FieldsEntity field) {
        ColumnBuilder comment = table.addColumn()
                .name(field.getName()).text().comment(field.getDisplayName());
